...
namespace CooS.Architectures {
	...
	public class InterruptManager {
		...
		static void SetGate(ushort flags, byte intno, Delegate handler) {
			...
			if(handler!=null) {
				...
				int target = Kernel.ObjectToValue(Assist.GetDelegateTarget(handler)).ToInt32();
				int entrypoint = Assist.GetFunctionPointer(handler).ToInt32();
				...
				assembler.Popad();
				assembler.Iretd();
				CooS.CodeModels.CodeInfo codeinfo = new CooS.CodeModels.CodeInfo(((System.IO.MemoryStream)assembler.BaseStream).ToArray(), 0);
		...
		public static void Register(int intno, InterruptHandler handler) {
			...
			SetGate(0xE00, (byte)intno, handler);
		}
