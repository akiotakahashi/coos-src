...
namespace CooS.CodeModels.CLI {
	...
	sealed class AssemblyDef : AssemblyBase {
		...
		private readonly TablesHeap tables;
		...
		public AssemblyDef(MetadataRoot mdroot) {
			this.mdroot = mdroot;
		...
		// For convinience
		public CooS.CodeModels.CLI.Signature.SignatureReader OpenSig(int rowIndex) {
			return new CooS.CodeModels.CLI.Signature.SignatureReader(this.Metadata.Blob.OpenData(rowIndex));
		}

		public override FieldInfoImpl GetFieldInfo(int rowIndex) {
			return this.GetFieldDef(rowIndex);
		}

		public override MethodInfoImpl GetMethodInfo(int rowIndex) {
			return this.GetMethodDef(rowIndex);
		}
		...
		public MemberInfo ResolveMember(MDToken token) {
			switch(token.TableId) {
			case TableId.Field:
				return this.GetFieldDef(token.RID);
			case TableId.Method:
				return this.GetMethodDef(token.RID);
		...
		public FieldInfoImpl ResolveField(MDToken token) {
			return (FieldInfoImpl)this.ResolveMember(token);
		}

		public MethodInfoImpl ResolveMethod(MDToken token) {
			return (MethodInfoImpl)this.ResolveMember(token);
		}

		public AssemblyBase ResolveAssembly(AssemblyRefRow asmref) {
		...
		CooS.Collections.TrieTree nametree = null;
		...
		*/
		...
		public FieldDefInfo GetFieldDef(int rowIndex) {
			if(rowIndex<0) throw new ArgumentOutOfRangeException();
			if(rowIndex==0) return null;
			int arrIndex = rowIndex-1;
		...
		public MethodDefInfo GetMethodDef(int rowIndex) {
			if(rowIndex<0) throw new ArgumentOutOfRangeException();
			if(rowIndex==0) return null;
			int arrIndex = rowIndex-1;
		...
		public FieldInfoImpl GetFieldRef(int rowIndex, bool throwOnMiss) {
			return (FieldInfoImpl)this.GetMemberRef(rowIndex,throwOnMiss);
		}

		public MethodInfoImpl GetMethodRef(int rowIndex, bool throwOnMiss) {
			return (MethodInfoImpl)this.GetMemberRef(rowIndex,throwOnMiss);
		}

		private FieldDefInfo LoadFieldDef(int rowIndex) {
			FieldRow row = (FieldRow)this.tables[TableId.Field][rowIndex];
			return new FieldDefInfo(this, row);
		}

		private MethodDefInfo LoadMethodDef(int rowIndex) {
		...
		class FieldCollection : ICollection {
			...
			class FieldEnumerator : IEnumerator {
				...
				public object Current {
					get {
						return this.col.assembly.GetFieldDef(this.currentRow);
					}
				}
		...
		class MethodCollection : ICollection {
			...
			class MethodEnumerator : IEnumerator {
				...
				public object Current {
					get {
						return this.col.assembly.GetMethodDef(this.currentRow);
					}
				}
