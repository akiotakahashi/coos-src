...
namespace CooS.CodeModels {
	...
	public abstract class Assembler {
		...
		private MethodInfoImpl RefAllocateObjectImpl {
			get {
				if(allocateobjectimpl==null) {
					allocateobjectimpl = this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:AllocateObjectImpl",true);
			...
			if(constructor.DeclaringType.IsValueType) {
				...
				if(0!=(constructor.GetMethodImplementationFlags()&MethodImplAttributes.Runtime)) {
					if(constructor.DeclaringType.IsSubclassOf(typeof(Delegate))) {
						...
						this.Call(this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:AllocateDelegate",true));
					} else {
		...
		public void AllocateSzArray(TypeImpl type) {
			...
			this.Call(this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:AllocateSzArrayImpl",true));
		}
		...
		public void LoadString(int sid) {
			...
			this.Call(this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:LoadStringImpl",true));
		}
		...
		public void Box(TypeImpl type) {
			this.LoadConstant(type.AssemblyInfo.Id);
			this.LoadConstant(type.RowIndex);
			this.Call(this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:BoxImpl",true));
		...
		public void IsInst(TypeImpl type) {
			this.LoadConstant(type.AssemblyInfo.Id);
			this.LoadConstant(type.RowIndex);
			this.Call(this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:IsInstImpl",true));
		}
		...
		public void CastClass(TypeImpl type) {
			...
			this.LoadConstant(type.AssemblyInfo.Id);
			this.LoadConstant(type.RowIndex);
			this.Call(this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:CastClassImpl",true));
		}
		...
		private MethodInfoImpl RefPrepareExecutableCode {
			get {
				if(prepareexecutablecode==null) {
					prepareexecutablecode = this.Method.Assembly.Manager.ResolveMethod("CooS.CodeModels.Assembler:PrepareExecutableCode",true);
		...
		private static MethodInfoImpl RefFindActualMethod {
			get {
				if(findactualmethod==null) {
					findactualmethod = CooS.Management.AssemblyResolver.Manager.ResolveMethod("CooS.CodeModels.Assembler:FindActualMethod",true);
		...
		private static MethodInfoImpl RefFindActualMethodForInterface {
			get {
				if(findactualmethod4if==null) {
					findactualmethod4if = CooS.Management.AssemblyResolver.Manager.ResolveMethod("CooS.CodeModels.Assembler:FindActualMethodForInterface",true);
		...
		public static void CompileTest() {
			MethodInfoImpl m = CooS.Management.AssemblyResolver.Manager.ResolveMethod("CooS.CodeModels.CLI.MethodDefInfo:GenerateExecutableCode",true);
